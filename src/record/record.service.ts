...
export class RecordService {
    ...
    async getRecordByIdDentist(id: string): Promise<Record[]> {
        ...
        const response = await Promise.all(records.map(async (item) => {
            const precription = await this.prismaService.prescription.findMany({
                where: {
                    idRecord: item.id
                }
            });
            const serviceIndicator = await this.prismaService.serviceIndicator.findMany({
                where: {
                    idRecord: item.id
                }
            });
            return { ...item, precription, serviceIndicator };
        }));
        return response;
    }
    ...
    async getRecordById(id: string): Promise<any> {
        ...
        const serviceIndicators = await this.prismaService.serviceIndicator.findMany({
            where: {
                idRecord: record.id
            }
        });
    ...
    async getPrecriptionById(id: string): Promise<Prescription[]> {
        ...
        if ((await response).length === 0) throw new HttpException({ message: 'Prescription not found' }, HttpStatus.NOT_FOUND);
        return response;
    }
    ...
    async getServiceIndicatorById(id: string): Promise<ServiceIndicator[]> {
        ...
        if ((await response).length === 0) throw new HttpException({ message: 'Service Indicator not found' }, HttpStatus.NOT_FOUND);
        return response;
    }
    ...
    async addRecord(data: any): Promise<any> {
        ...
        if (existingRecord) {
            throw new HttpException({ message: 'Record already exists' }, HttpStatus.BAD_REQUEST);
        }
        else {
    ...
    async addPrecription(data: any): Promise<any> {
        ...
        if (!drug) {
            throw new HttpException({ message: 'Drug not found' }, HttpStatus.NOT_FOUND);
        }
        ...
        if (alreadyPrescription) {
            throw new HttpException({ message: 'Drug already exists' }, HttpStatus.BAD_REQUEST);
        }
    ...
    async addServiceIndicator(data: any): Promise<any> {
        ...
        const alreadyServiceIndicator = await this.prismaService.serviceIndicator.findFirst({
            where: {
                service: {
        ...
        if (alreadyServiceIndicator) throw new HttpException({ message: 'Service already exists' }, HttpStatus.BAD_REQUEST);
        else {
    ...
    async deleteRecord(data: any): Promise<any> {
    ...
    async deleteDrugPrecription(data: any): Promise<any> {
        const drug = await this.prismaService.drug.findFirst({
            where: {
                OR: [
                    {
        ...
        if (!prescription) {
            throw new HttpException({ message: 'Prescription not found' }, HttpStatus.NOT_FOUND);
        }
    ...
    async deletePrecription(data: any): Promise<any> {
    ...
    async deleteServiceInServiceIndicator(data: any): Promise<any> {
        const service = await this.prismaService.service.findFirst({
            where: {
                OR: [
                    { id: data.idService },
        ...
        const record = await this.prismaService.record.update({
            ...
            data: {
                total: {
                    decrement: data.quantity * service.price
                }
            }
        });
        const response = await Promise.all([record, serviceIndicator]);
    ...
    async deleteServiceIndicator(data: any): Promise<any> {
        const serviceIndicator = await this.prismaService.serviceIndicator.deleteMany({
            where: {
                idRecord: data.id
            }
        });
